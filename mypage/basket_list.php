...
$sql = "SELECT count(*) FROM basket where mb_id = '".$mb_id."'";
...
$sql2 = "SELECT * FROM basket join product on basket.p_id= product.p_id where mb_id = '".$mb_id."' order by b_id asc limit ".$page_start.",".$list."";
...
<head>
    ...
    <link rel="stylesheet" href="../css/basket_list.css?after">
    ...
    <script src="../js/checkbox.js?after"></script>
...
<div class="wrap">
    ...
    <div class="main">
            <div class="mainContent">
                <h1 class="titleText1">장바구니 목록</h1>
                <div class="section">
                <form action="../order.php" name="f" method="post">
                    <table>
                        <tr><th>상품</th><th>이름</th><th>가격</th><th>수량</th></tr>
                    <?php while($row = mysqli_fetch_array($result)) {
                            ?>
                        <tr>
                        <td><input type="checkbox" class="chk_box" id=<?php echo "c".$i;?> name="chk[]" value=<?php echo $row['b_id'];?>><img class="img_box" src="<?php echo ".".$row['img_path'].$row['p_id']?>.jpg" alt=""></td>
                    ...
                    <div class = "btn_box">
                        ...
                        <input type="hidden" name="delete_basket" value="y">
                        <input type='checkbox' name='chk[]' value="selectAll" onclick='selectAll(this)'/>  전체 선택
                        <input type="button" class="btn1" value="선택 삭제" onclick="historyDel()">
                        <input type="submit" class="btn1" value="선택 구매">
                    </div>
                </form>
                </div>
                <div class="section2">
                <?php
                    ...
                    if ($page > 1){
                            echo "<a href='./basket_list.php?page=1'><i class='fas fa-angle-double-left'></i></a>";
                            $pre = $page -1;
                            echo "<a href='./basket_list.php?page=".$pre."'><i class='fas fa-angle-left'></i></a>";
                    }
                    else {
                        echo "<a href='./basket_list.php?page=1'><i class='fas fa-angle-double-left'></i></a>";
                        echo "<a href='./basket_list.php?page=1'><i class='fas fa-angle-left'></i></a>";
                    }
                    ...
                    for($i = $block_start; $i <= $block_end; $i++){
                        if($page == $i){
                            ...
                            echo "<a href='./basket_list.php?page=".$i."'><span style='font-size : 2em'>".$i."</span></a>";
                        }
                    }

                    if($page < $total_page){
                        $next = $page +1;
                            echo "<a href='./basket_list.php?page=".$next."'><i class='fas fa-angle-right'></i></a>";
                            echo "<a href='./basket_list.php?page=".$total_page."'><i class='fas fa-angle-double-right'></i></a>";
                    }
                    else {
                        echo "<a href='./basket_list.php?page=".$total_page."'><i class='fas fa-angle-right'></i></a>";
                        echo "<a href='./basket_list.php?page=".$total_page."'><i class='fas fa-angle-double-right'></i></a>";
                    }
                    mysqli_close($conn);
                    ?>
                ...
                <div class="section3">
                    <input type="hidden" name="buy_option" value="basket_all">
                    <input type="button" class="btn2" value="계속 쇼핑하기" onclick= "location.href ='../product.php?menu=ice'">
                    <input type="submit" class="btn3" value="전체 구매하기">
                </div>
                </form>
            </div>
            <div class="sidebar">
